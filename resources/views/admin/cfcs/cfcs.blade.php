...
<html lang="pt-br">
    ...
    <meta charset="utf-8">
        ...
        <!-- Title / Icon -->
        <title>Gestão-CFC - CFCs-ADM</title>
                ...
                <div class="carta">
                    <div class="card">
                        <div class="card-content">
                            <span class="card-title activator grey-text text-darken-4">CFCs</span>
                            <button id="btn-cadastrar" class="btn waves-effect waves-light blue-grey darken-3" type="submit" name="action">
                                <a href="{{ route('cfc.create') }}" class="white-text">Cadastrar</a>
                        ...
                        <div class="row" id="filtro">
                            <form action="{{ route('cfc.search') }}" method="POST" class="col s6">
                                @csrf
                                ...
                                <tbody>
                                    @forelse($cfcs as $cfc)
                                        <tr>
                                            ...
                                            <th>{{ $cfc->cnpj }}</th>
                                            <th>{{ $cfc->celular }}</th>
                                            <th>{{ $cfc->razão_social }}</th>
                                            <th>
                                                <button class="btn waves-effect waves-light blue-grey darken-3">
                                                    <a class="white-text" href="{{ route('cfc.edit', $cfc->id) }}">
                                                        <i class="small material-icons">edit</i>
                                                    </a>
                                                </button>
                                            </th>
                                            <th>
                                                <form action="{{ route('cfc.destroy', $cfc->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn waves-effect waves-light blue-grey darken-3">
